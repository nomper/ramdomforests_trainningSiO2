...
for f=replace(FL,".treated.mat","")
    ...
    for f=FL
        ...
        ExpectData=vertcat(ExpectData, a.mapping_p_lt_smoothed_max_Energy(:));
    end
...
%     view(DefaultTree,'Mode','Graph')
    ...
    a.mapping_p_lt_smoothed_max_Energy = a.mapping_p_lt_smoothed_max_Energy.*area;
    MaxPos=max([pre2_map(:);a.mapping_p_lt_smoothed_max_Energy(:)])
    
    f1=figure(1);imagesc(pre2_map)
    colorbar;
    % f.CurrentAxes.CLim=[0 0.25];
    f1.CurrentAxes.CLim=[1.5 2.0];
    title("Prediction Max Inteisty Position")
    print(strcat(samplename,".predict.png"),"-dpng","-r250");
    %     f1.CurrentAxes.CLim=[0.15 0.35];
    
    f2=figure(2);imagesc(a.mapping_p_lt_smoothed_max_Energy)
    colorbar;
    f2.CurrentAxes.CLim=[1.5 2.0];
    title("Experiment Max Inteisty Position")
