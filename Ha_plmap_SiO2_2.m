...
if(exist(strcat(samplename,".mat"),"file")>0)
    ...
    %     "each load"
    FilenameRT = replace("RT/005PLWSe2SiO2S2300K633nm2uW15s_%.0f_1.txt","/","//");
    FilenameLT = replace("LT/002PLWSe2SiO2S115K633nm15uW15s_%.0f_1.txt","/","//");
    pickupLT = 571:611;
    pickupRT = 570:740;
    ...
    parfor f = 1:X*Y
        ...
        Inteinsity_sigm_rt(:,f) = a(pickupRT,3);
        Inteinsity_sigp_rt(:,f) = a(pickupRT+1340,3);
        
        a = load(sprintf(FilenameLT,f));
        Inteinsity_sigm_lt(:,f) = a(pickupLT,3);
...
%cosmic ray cut
Inteinsity_sigm_rt(Inteinsity_sigm_rt>320) = 0;
Inteinsity_sigp_rt(Inteinsity_sigp_rt>300) = 0;
Inteinsity_sigm_lt(Inteinsity_sigm_lt>850) = 0;
Inteinsity_sigp_lt(Inteinsity_sigp_lt>850) = 0;
...
mapping_m_lt_max_Energy(or(mapping_m_lt_max_Energy<1.72,1.77<mapping_m_lt_max_Energy)) = 0;
mapping_p_lt_max_Energy(or(mapping_p_lt_max_Energy<1.72,1.77<mapping_p_lt_max_Energy)) = 0;
...
locate_of_zero_lt = ~((mapping_m_lt)&(mapping_p_lt)&(mapping_m_lt_max)&(mapping_p_lt_max));
...
f=figure(3);imagesc(mapping_m_lt);colorbar;title("\sigma- @LT");f.CurrentAxes.CLim = [0 5000];print_en(print_flag,"-f3","mapping_m_lt.png","-dpng","-r250");
f=figure(4);imagesc(mapping_p_lt);colorbar;title("\sigma+ @LT");f.CurrentAxes.CLim = [0 5000];print_en(print_flag,"-f4","mapping_p_lt.png","-dpng","-r250");
